...
WizFiRingBuffer WizFi310Drv::ringBuf(RING_BUFFER_SIZE);
...
uint16_t WizFi310Drv::availData()
{
    ...
    startMillis = millis();
    do {
		if (!WizFi310Serial->available() || ringBuf.isFull()) {
...
int WizFi310Drv::getDataBuf(uint8_t connId, uint8_t *buf, unsigned int bufSize)
{
	int ch;
...
int WizFi310Drv::sendCmd(const __FlashStringHelper* cmd, int timeout, ...)
{
	...
	char responseBuf[CMD_BUFFER_SIZE];
