...
void VS1003::begin(){
  ...
  setVolume(VOL, VOL); //Set initial volume
...
int VS1003::playStreamingAudio(WiFiClient _client, uint16_t _cur, uint16_t _length){
	WifiClient = _client;
	cur = _cur;
	length = _length;
	need_data = TRUE;
	...
	while (cur < length) {
		...
		Serial.println(F("Playing manually"));
		while(true){
		  noInterrupts();
		  if(needData()){
			fillBuffer();
...
void VS1003::pause(){
  Serial.println(F("Pausing current song..."));
  playing = false;
};

void VS1003::play(){
  if(playing == false){
    Serial.println(F("Playing current song..."));
    playing = true;
    while (!needData());
    fillBuffer();
  }
...
void VS1003::stop(){
  playing = false;
  streamingFinished = true;
  Serial.println(F("Streamming stopped..."));
};
...
void VS1003::useInterrupt(){
  ...
  if(irq == -1)
  {
    Serial.println(F(" it not interupt compatable"));
